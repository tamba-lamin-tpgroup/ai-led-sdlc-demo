    ...
    <article className={cn("group rounded-xl border border-border bg-white overflow-hidden shadow-sm hover:shadow-md transition-shadow", className)}>
      ...
      <div className="p-4">
        ...
        <div className="mt-3 flex items-center justify-between">
          <div className="flex items-center gap-1" aria-label={`${t("attraction.rating")}: ${formatRating(attraction.rating)} out of 5`}>
            <Star size={14} className="fill-brand-primary text-brand-primary" aria-hidden="true" />
            <span className="text-sm font-medium text-text">{formatRating(attraction.rating)}</span>
            <span className="text-xs text-text-muted">({attraction.reviewCount} {t("attraction.reviews")})</span>
          </div>
          <Link
            to={href}
